...
Eight real UIs built on the client, each exercising a different surface of the reactive-cache-plus-hooks model. All of them share a WS proxy for local MCP server access.
...
| **[composer](https://github.com/johnhenry/mcp-query/tree/main/apps/composer)** | React + Vite | A chat composer where the *user* drives MCP tools to assemble grounded input, with a pluggable model picker ([aimatey](https://github.com/johnhenry/aimatey)). |
